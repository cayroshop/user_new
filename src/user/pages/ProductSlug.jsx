  ...
  const getCategory = async () => {
    ...
    try {
      ...
      if (data.Product.hsn && data.Product.variations) {
        try {
          const { data: hsnData } = await axiosInstance.get(
            `/products-variations-hsn/${data.Product.hsn}`
          );
            ...
            Object.entries(productVariations).forEach(([title, values]) => {
              ...
              if (existingVariant) {
                existingVariant.values.push(
                  ...values.map((variant) => variant[title])
                );
              } else {
                acc.push({
                  title,
                  values: values.map((variant) => variant[title]),
                });
              }
            });
            return acc;
